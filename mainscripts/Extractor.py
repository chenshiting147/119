...
class ExtractSubprocessor(Subprocessor):
    ...
    class Cli(Subprocessor.Cli):
        ...
        def on_initialize(self, client_dict):
            ...
            self.final_output_path = Path(
                client_dict['final_output_dir']) if 'final_output_dir' in client_dict.keys() else None
                ...
                if src_dflimg is not None and len(rects) != 1:
                    ...
                    for rect, image_landmarks in zip(rects, landmarks):
                        ...
                        if self.face_type == FaceType.MARK_ONLY:
                            ...
                            image_to_face_mat = LandmarksProcessor.get_transform_mat(image_landmarks, self.image_size,
                                                                                     self.face_type)
                            face_image = cv2.warpAffine(image, image_to_face_mat, (self.image_size, self.image_size),
                                                        cv2.INTER_LANCZOS4)
                            face_image_landmarks = LandmarksProcessor.transform_points(image_landmarks,
                                                                                       image_to_face_mat)

                            landmarks_bbox = LandmarksProcessor.transform_points(
                                [(0, 0), (0, self.image_size - 1), (self.image_size - 1, self.image_size - 1),
                                 (self.image_size - 1, 0)], image_to_face_mat, True)
                        ...
                        if self.debug_dir is not None:
                            LandmarksProcessor.draw_rect_landmarks(debug_image, rect, image_landmarks, self.image_size,
                                                                   self.face_type, transparent_mask=True)
    ...
    # override
    def __init__(self, input_data, type, image_size=None, face_type=None, debug_dir=None, multi_gpu=False,
                 cpu_only=False, manual=False, manual_window_size=0, final_output_path=None):
        ...
        if not self.manual:
            ...
            while len(self.input_data) > 0:
                ...
                if need_remark_face:  # need remark image from input data that already has a marked face?
                    need_remark_face = False
                    if len(
                            data_rects) != 0:  # If there was already a face then lock the rectangle to it until the mouse is clicked
...
def extract_fanseg(input_dir, device_args={}):
    ...
    if paths_to_extract_len > 0:
        ...
        data = ExtractSubprocessor([ExtractSubprocessor.Data(filename) for filename in paths_to_extract], 'fanseg',
                                   multi_gpu=multi_gpu, cpu_only=cpu_only).run()
...
def extract_umd_csv(input_file_csv,
    ...
    for d in csv_data:
        ...
        data += [
            ExtractSubprocessor.Data(filename=filename, rects=[[x, y, x + w, y + h]], pitch_yaw_roll=pitch_yaw_roll)]
    ...
    if len(data) > 0:
        ...
        data = ExtractSubprocessor(data, 'final', image_size, face_type, None, multi_gpu=multi_gpu, cpu_only=cpu_only,
                                   manual=False, final_output_path=output_path).run()
...
def main(input_dir,
    ...
    if debug_dir is not None:
        ...
        if manual_output_debug_fix:
            ...
            input_path_image_paths = DeletedFilesSearcherSubprocessor(input_path_image_paths,
                                                                      Path_utils.get_image_paths(
                                                                          debug_output_path)).run()
    ...
    if images_found != 0:
        if detector == 'manual':
            ...
            data = ExtractSubprocessor([ExtractSubprocessor.Data(filename) for filename in input_path_image_paths],
                                       'landmarks', image_size, face_type, debug_dir, cpu_only=cpu_only, manual=True,
                                       manual_window_size=manual_window_size).run()
        else:
            io.log_info('执行第一次通过...')
            data = ExtractSubprocessor([ExtractSubprocessor.Data(filename) for filename in input_path_image_paths],
                                       'rects-' + detector, image_size, face_type, debug_dir, multi_gpu=multi_gpu,
                                       cpu_only=cpu_only, manual=False).run()

            io.log_info('执行第二次通过...')
            data = ExtractSubprocessor(data, 'landmarks', image_size, face_type, debug_dir, multi_gpu=multi_gpu,
                                       cpu_only=cpu_only, manual=False).run()

        io.log_info('执行第三次通过...')
        data = ExtractSubprocessor(data, 'final', image_size, face_type, debug_dir, multi_gpu=multi_gpu,
                                   cpu_only=cpu_only, manual=False, final_output_path=output_path).run()
        faces_detected += sum([d.faces_detected for d in data])
        ...
        if manual_fix:
            if all(np.array([d.faces_detected > 0 for d in data]) == True):
                ...
                fix_data = ExtractSubprocessor(fix_data, 'landmarks', image_size, face_type, debug_dir, manual=True,
                                               manual_window_size=manual_window_size).run()
                fix_data = ExtractSubprocessor(fix_data, 'final', image_size, face_type, debug_dir, multi_gpu=multi_gpu,
                                               cpu_only=cpu_only, manual=False, final_output_path=output_path).run()
